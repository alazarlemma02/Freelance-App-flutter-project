  ...
  Widget build(BuildContext context) {
    return BlocBuilder<UserBloc, UserState>(
      builder: (context, state) {
                ...
                child: Column(
                  children: [
                          ...
                          children: <Widget>[
                            Padding(
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  children: [
                                     UserTypeAndSkill(fullName: state.user.fullName.toString(), category: state.user.category.toString(),skill: state.user.skillLevel.toString(),),
                                    Container(
                                      ...
                                      width:
                                          MediaQuery.of(context).size.height *
                                              0.4,
                                      child: Text(
                                        state.user.aboutYourself.toString(),
                                        style: TextStyle(
                                            color: CustomColors.blackTextColor),
                                      ),
                                    ),
                                    Container(
                                      ...
                                      width:
                                          MediaQuery.of(context).size.height *
                                              0.4,
                                      child: AttachmentFile(attachment:  state.user.attachmentUrl,),
                                    ),
                                  ],
                                ),
                              ),
                            ),
                            UserProfilePicture(profileImage: state.user.profileImage.toString(),),
                    ...
                    Container(
                     ...
                     ContactDetail(email: state.user.email, phoneNumber: state.user.phoneNumber, socialMedia: state.user.socialMediaLink),
                  ],
                ),
              ),
            ],
        ...
        } return Container(child: Center(child: Text('Some Error Ocurred', style: TextStyle(color: CustomColors.blackTextColor),),),);
      },
    );
  }
}
